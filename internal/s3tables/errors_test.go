...
func TestWrapError_SmithyAPIError(t *testing.T) {
	tests := []struct {
		...
		{"AccessDenied", "AccessDenied", ErrorTypeForbidden},
		{"ValidationException", "ValidationException", ErrorTypeBadRequest},
		{"ServiceException", "ServiceException", ErrorTypeInternalServer},
		{"UnrecognizedClientException", "UnrecognizedClientException", ErrorTypeCredentials},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			apiErr := &mockAPIError{code: tt.code, message: "test error"}
